...
// Bersihkan entry expired secara berkala — agar Map tidak tumbuh tanpa batas
// di server long-running (Koyeb). Jalan setiap 10 menit, non-blocking.
setInterval(() => {
  const now = Date.now();
  for (const [id, expiresAt] of deadStreamIds) {
    if (expiresAt <= now) deadStreamIds.delete(id);
  }
}, 10 * 60 * 1000).unref();
...
/* ── Strict allowlists ──
   NOTE: vdy.to pindah domain ke streamrizz.com (301 permanen, path dipertahankan).
   Thumbnail kini disajikan dari i.streamrizz.com / streamrizz.com/image —
   domain lama i.vdy.to hanya 301-redirect (proxy thumbnail tidak ikut redirect). */
const THUMB_HOSTS  = new Set(['i.vdy.to', 'i.streamrizz.com', 'streamrizz.com']);
...
router.get('/api/folder/:id', async (req, res) => {
  ...
  try {
    ...
    /* Deteksi soft-404: vdy.to kini redirect permanen ke streamrizz.com yang
       membalas HTTP 200 bahkan untuk halaman error "404 Page Not Found".
       Tanpa cek ini, folder kosong + judul "404 ..." dikirim ke client
       seolah-olah folder valid. */
    const rawPageTitle = $('title').text().trim();
    if (rawPageTitle.includes('404')) {
      return apiError(res, 404, 'Folder tidak ditemukan');
    }
...
router.get('/api/video/:id', async (req, res) => {
  ...
  try {
    // Step 1: scrape halaman /d/<id> untuk embedToken
    const { data: pageHtml } = await ax.get(`${BASE}/d/${id}`, {
      headers: { ...baseHeaders, 'Referer': `${BASE}/` },
    });

    const tokenMatch = pageHtml.match(/var\s+embedToken\s*=\s*['"]([^'"]+)['"]/);
    if (!tokenMatch) return apiError(res, 404, 'Sumber video tidak ditemukan');
    const embedToken = tokenMatch[1];

    // Step 2: decode JWT payload (format 2-part: payload.signature, part[0] = payload)
    let src = null, thumb = '', title = id;
    try {
      const payload = JSON.parse(Buffer.from(embedToken.split('.')[0], 'base64').toString('utf8'));
      if (payload.rf) src = `https://vidoycdn.b-cdn.net/${payload.rf}`;
      if (payload.im) thumb = `https://i.streamrizz.com/image/${payload.im}`;
      if (payload.ti) title = payload.ti;
    } catch (_) {}

    // Step 3: fallback ke stream.php jika JWT decode gagal atau domain tidak diizinkan
    if (!src || !allowedStreamUrl(src)) {
      const { data: playerHtml } = await ax.get(
        `${BASE}/stream.php?bucket=vidoycdn&id=${id}&t=${encodeURIComponent(embedToken)}`,
        { headers: { ...baseHeaders, 'Referer': `${BASE}/d/${id}` } }
      );
      const $p = cheerio.load(playerHtml);
      src = $p('source[type="video/mp4"]').attr('src')
         || $p('video source').attr('src')
         || $p('video').attr('src')
         || null;
      if (!src || !allowedStreamUrl(src)) {
...
router.get('/proxy/stream/:id', async (req, res) => {
  ...
  async function resolveP1Mp4(evictFirst = false) {
    ...
    // Decode JWT payload (format 2-part: payload.signature, part[0] = payload)
    let src = null, thumb = '', title = id;
    try {
      const payload = JSON.parse(Buffer.from(embedToken.split('.')[0], 'base64').toString('utf8'));
      if (payload.rf) src = `https://vidoycdn.b-cdn.net/${payload.rf}`;
    ...
    // Fallback ke stream.php jika JWT decode gagal atau domain tidak diizinkan
    if (!src || !allowedStreamUrl(src)) {
      const { data: playerHtml } = await ax.get(
        `${BASE}/stream.php?bucket=vidoycdn&id=${id}&t=${encodeURIComponent(embedToken)}`,
        { headers: { ...baseHeaders, 'Referer': `${BASE}/d/${id}` } }
