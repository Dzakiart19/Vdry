...
/* ── Strict CDN allowlist ──
   vdn.bokepking.cam = direct MP4 CDN (mayoritas video).
   app.lintoq.com + stream.kingbokep.video = HLS gateway untuk subset video
   premium/baru (diamati via network capture, sample luas masih MP4).
   Jika .m3u8 terdeteksi, /proxy/bk/stream akan return 404 dgn pesan HLS
   yang jelas — frontend bk.html belum support HLS.js. TODO bila need:
   tambah /proxy/bk/hls + /seg + frontend HLS.js load (mirror rb/yb pattern). ── */
function isAllowedBkCdnUrl(raw) {
  try {
    const u = new URL(raw);
    if (u.protocol !== 'https:') return false;
    if (u.hostname === BK_CDN || u.hostname.endsWith('.' + BK_CDN)) return true;
    if (u.hostname === 'app.lintoq.com' || u.hostname === 'stream.kingbokep.video') {
      // HLS path — diterima tapi /proxy/bk/stream akan tolak (return 404 dgn pesan jelas)
      return true;
    }
...
async function resolveBkMp4(slug, evictFirst = false) {
  ...
  // HLS detection: jika URL berakhir .m3u8 atau dari lintoq/kingbokep.video,
  // /proxy/bk/stream belum support HLS — return null + log agar user dapat
  // error jelas. TODO: implement /proxy/bk/hls + frontend HLS.js bila muncul.
  if (mp4Url.endsWith('.m3u8') || mp4Url.includes('lintoq.com') || mp4Url.includes('kingbokep.video')) {
    logCdnAlert(`[cdn-alert] P4 HLS stream terdeteksi untuk "${slug}" — belum didukung, butuh frontend HLS.js`);
    return null;
  }
