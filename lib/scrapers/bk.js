...
router.get('/api/bk/posts', async (req, res) => {
  ...
  try {
    ...
    const result = { posts: assembled, totalPages };
    bkPostsCache.set(key, result);
    res.json(result);

  } catch (err) {
    console.error('[bk] posts error:', err.message);
    if (err.response?.status === 400) return apiError(res, 400, 'Parameter tidak valid');
...
async function resolveBkMp4(slug, evictFirst = false) {
  ...
  bkVideoUrlCache.set(slug, { mp4Url, title, thumb });
  return mp4Url;
}
...
module.exports = { router, caches: [bkPostsCache, bkVideoUrlCache, bkThumbCache] };
