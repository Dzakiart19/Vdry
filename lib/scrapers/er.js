...
/* ── Valid params ── */
const VALID_SORT = new Set(['hot', 'new']);
const VALID_CAT  = new Set(['all', 'straight', 'trans', 'gay', 'hentai']);

/* ── Build listing URL from sort + cat + page + query ──
   cat=all  sort=hot → /explore?page=N
   cat=all  sort=new → /explore/new?page=N
   cat=X    sort=hot → /version/{X}?page=N
   cat=X    sort=new → /version/{X}/new?page=N
   search             → /search?q=QUERY&page=N (cat+sort ignored)
────────────────────────────────────────────────────────────────────────── */
function buildErUrl(sort, cat, page, q) {
  ...
  return sort === 'new'
    ? `${ER_BASE}/version/${cat}/new?page=${page}`
    : `${ER_BASE}/version/${cat}?page=${page}`;
}
...
router.get('/api/er/posts', async (req, res) => {
  ...
  const sort = VALID_SORT.has(req.query.sort) ? req.query.sort : 'hot';
  const cat  = VALID_CAT.has(req.query.cat)   ? req.query.cat  : 'all';
  const key  = `${page}:${q}:${sort}:${cat}`;
  ...
  try {
    const url = buildErUrl(sort, cat, page, q);
