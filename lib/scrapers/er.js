...
/* ── Kategori statis erome (version filter + sort:new) ── */
const ER_CATEGORIES = [
  { id: 'straight', slug: 'straight', name: 'Straight' },
  { id: 'trans',    slug: 'trans',    name: 'Trans'     },
  { id: 'gay',      slug: 'gay',      name: 'Gay'       },
  { id: 'hentai',   slug: 'hentai',   name: 'Hentai'    },
  { id: 'new',      slug: 'new',      name: 'New'       },
];

/* ── Map catId → { sort, version } ──
   catId='new'       → sort=new, version=all
   catId='straight'  → sort=hot, version=straight
   catId='' / null   → sort=hot, version=all
── */
function resolveSortVersion(catId) {
  if (catId === 'new') return { sort: 'new', version: 'all' };
  const version = ['straight', 'trans', 'gay', 'hentai'].includes(catId) ? catId : 'all';
  return { sort: 'hot', version };
}

/* ── Build listing URL ──
   version=all  sort=hot → /explore?page=N
   version=all  sort=new → /explore/new?page=N
   version=X    sort=hot → /version/{X}?page=N
   version=X    sort=new → /version/{X}/new?page=N
   search                → /search?q=QUERY&page=N
─────────────────────────────────────────────────── */
function buildErUrl(sort, version, page, q) {
  if (q) return `${ER_BASE}/search?q=${encodeURIComponent(q)}&page=${page}`;
  if (version === 'all') {
    return sort === 'new'
      ? `${ER_BASE}/explore/new?page=${page}`
      : `${ER_BASE}/explore?page=${page}`;
  }
  return sort === 'new'
    ? `${ER_BASE}/version/${version}/new?page=${page}`
    : `${ER_BASE}/version/${version}?page=${page}`;
}

/* ════════════════════════════════════════════════════════════
   CATEGORIES (statis — versi erome)
...
router.get('/api/er/posts', async (req, res) => {
  ...
  const { sort, version } = resolveSortVersion(catId);
  const key   = `${page}:${q}:${sort}:${version}`;
  ...
  try {
    const url = buildErUrl(sort, version, page, q);
