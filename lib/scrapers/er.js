...
/* Thumbnail domain — s{N}.erome.com (Ceph/S3 image CDN) */
const isAllowedErThumbUrl = isAllowedErUrl;

/* ── Rewrite raw s*.erome.com thumb URL → /proxy/er/thumb?url=... ── */
function proxyThumb(raw) {
  if (!raw) return '';
  if (!isAllowedErThumbUrl(raw)) return raw;
  return `/proxy/er/thumb?url=${encodeURIComponent(raw)}`;
}
...
const erThumbCache = makeCache(1000, 24 * 60 * 60 * 1000, 'er_thumb');

/* ── Kategori erome — search-based (version filter sudah mati, semua redirect /explore) ──
   catId=''        → /explore?page=N          (HOT default)
   catId='new'     → /explore/new?page=N      (BARU)
   catId lain      → /search?q=catId&page=N   (search keyword)
── */
const ER_CATEGORIES = [
  { id: 'new',      name: 'New'       },
  ...
  { id: 'big ass',  name: 'Big Ass'   },
  { id: 'blowjob',  name: 'Blowjob'   },
  { id: 'anal',     name: 'Anal'      },
];

/* ── Build listing URL ──
   catId=''    q=''    → /explore?page=N
   catId='new' q=''    → /explore/new?page=N
   catId=X     q=''    → /search?q=X&page=N   (search keyword)
   q=QUERY     (any)   → /search?q=QUERY&page=N
─────────────────────────────────────────────────── */
function buildErUrl(catId, page, q) {
  if (q) return `${ER_BASE}/search?q=${encodeURIComponent(q)}&page=${page}`;
  if (!catId) return `${ER_BASE}/explore?page=${page}`;
  if (catId === 'new') return `${ER_BASE}/explore/new?page=${page}`;
  return `${ER_BASE}/search?q=${encodeURIComponent(catId)}&page=${page}`;
}

/* ════════════════════════════════════════════════════════════
   CATEGORIES (search-based)
   GET /api/er/categories
════════════════════════════════════════════════════════════ */
router.get('/api/er/categories', (_req, res) => {
  res.json(ER_CATEGORIES);
});

/* ════════════════════════════════════════════════════════════
   LISTING / POSTS
   GET /api/er/posts?p=N&q=QUERY&cat=CATID
════════════════════════════════════════════════════════════ */
router.get('/api/er/posts', async (req, res) => {
  ...
  const catId = (req.query.cat || '').trim();
  ...
  try {
    const url = buildErUrl(catId, page, q);
    ...
    $('a.album-link').each((_, el) => {
      ...
      const thumb = proxyThumb($a.find('img.album-thumbnail.active').first().attr('src') || '');
...
async function resolveErVideo(albumId, evictFirst = false) {
  ...
  $('a.album-link').each((_, el) => {
    ...
    seenRel.add(m[1]);
    const rId    = m[1];
    const rThumb = proxyThumb($a.find('img.album-thumbnail.active').first().attr('src') || '');
...
/* ════════════════════════════════════════════════════════════
   THUMB PROXY
   GET /proxy/er/thumb?url=...
   Browser tidak pernah melihat URL CDN asli (s{N}.erome.com).
   Cache 24 jam di erThumbCache — thumbnail tidak berubah.
════════════════════════════════════════════════════════════ */
router.get('/proxy/er/thumb', async (req, res) => {
  const raw = req.query.url;
  if (!raw || !isAllowedErThumbUrl(raw)) return res.status(400).end();

  const cached = erThumbCache.get(raw);
  if (cached) {
  ...
  try {
    const upstream = await axNoRedirect.get(raw, {
      headers:      { 'User-Agent': UA, 'Referer': `${ER_BASE}/` },
      responseType: 'arraybuffer',
    });
    const ct = upstream.headers['content-type'] || '';
    if (!ct.startsWith('image/')) return res.status(415).end();

    erThumbCache.set(raw, { ct, buf: Buffer.from(upstream.data) });
    ...
    console.error('[er] thumb proxy error:', err.message);
...
module.exports = { router, caches: [erPostsCache, erVideoCache, erThumbCache] };
