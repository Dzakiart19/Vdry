...
router.get('/api/vd/posts', async (req, res) => {
  ...
  try {
    ...
    // videy.design tidak menampilkan total halaman dan hanya ada tombol Next (tanpa nomor).
    // Deteksi keberadaan link "next page" dari pagination — lazy: jika ada next → page+1,
    // jika tidak ada → page ini adalah yang terakhir. Akurat tanpa angka halaman total.
    const hasNextLink = $('a.pagination-btn[href*="page="]').toArray().some(el => {
      const href = $(el).attr('href') || '';
      const m    = href.match(/[?&]page=(\d+)/);
      return m && parseInt(m[1]) > page;
    });
    const totalPages = hasNextLink ? page + 1 : page;
