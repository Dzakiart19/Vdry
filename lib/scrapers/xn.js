...
/* ── API server list (from xchina.tube httpNames in bundle) ──
   2026-08: v2.cdn199.com ditambahkan di posisi pertama — server yang
   dipakai situs aslinya saat ini. ── */
const XN_SERVERS = [
  'https://v2.cdn199.com',
...
/* ── Envelope deviceInfo untuk gateway /js (dicontek dari request situs
   asli — server hanya butuh field-nya ada, isinya fleksibel) ── */
const XN_DEVICE_INFO = {
  model: 'X11', platform: 'web', operatingSystem: 'unknown',
  osVersion: 'x86.64', manufacturer: 'Google Inc.',
  isVirtual: false, webViewVersion: '138.0.0.0',
};
...
/* ── POST ke API server dengan AES decrypt, fallback ke server lain ──
   2026-08: xchina.tube memindahkan SEMUA REST path (sevenVideos,
   searchSevenVideos, relatedSevenVideos, dst.) ke gateway tunggal
   POST /js. Path asli sekarang dikirim di field "url" dalam body JSON
   (envelope: deviceInfo/app/isStandalone/theLink/uuid + parameter asli
   merger di level top-level). Response masih {r: <AES-CBC base64>}
   dengan key "xxx" — format enkripsi tidak berubah. ── */
async function apiPost(urlPath, body = {}) {
  let lastErr;
  for (const server of XN_SERVERS) {
    try {
      const { data } = await axXn.post(`${server}/js`, {
        deviceInfo:     XN_DEVICE_INFO,
        app:            'xchina.tube',
        isStandalone:   false,
        theLink:        'novaluenull',
        uuid:           'novalue',
...
// p8_categories: 6 jam (hardcoded list, jarang berubah)
...
const xnCategoriesCache = makeCache(1,   6 * 60 * 60 * 1000, 'p8_categories');

module.exports = { router, caches: [xnPostsCache, xnM3u8Cache, xnVideoCache, xnFreshCache, xnCategoriesCache] };

/* ── Real category list (22 named collections per-studio, verified dari
   sidebar xchina.tube asli). Parameter `type=c0..c21` dikirim ke
   `/sevenVideos?page=N&type=cN` untuk filter per-studio collection. ── */
const XN_CATEGORIES = [
  { id: 'c0',  name: 'Porn Videos'        },
  { id: 'c1',  name: 'Chinese Original'   },
  { id: 'c2',  name: 'Madou Media'         },
  { id: 'c3',  name: '91 Studio'           },
  { id: 'c4',  name: 'HongKongDoll'        },
  { id: 'c5',  name: 'LoveLive!'           },
  { id: 'c6',  name: 'Sugar Vlog'          },
  { id: 'c7',  name: 'Tianmei Media'        },
  { id: 'c8',  name: 'Peach Media'          },
  { id: 'c9',  name: 'Royal Chinese'       },
  { id: 'c10', name: 'Star Media'          },
  { id: 'c11', name: 'JD Films'            },
  { id: 'c12', name: 'Adult Headlines'     },
  { id: 'c13', name: 'Crow Media'          },
  { id: 'c14', name: 'Happy Media'         },
  { id: 'c15', name: 'Japan AV'            },
  { id: 'c16', name: 'Banana Fever'       },
  { id: 'c17', name: 'Taiwan Swag'         },
  { id: 'c18', name: 'Torture'             },
  { id: 'c19', name: 'China BDSM'          },
  { id: 'c20', name: 'Pissvids (wet)'      },
  { id: 'c21', name: 'My Favorites'        },
];
...
/* ── XN: Categories (hardcoded, no API endpoint exists for this) ── */
router.get('/api/xn/categories', (_req, res) => {
  const cached = xnCategoriesCache.get('list');
  if (cached) return res.json(cached);
  xnCategoriesCache.set('list', XN_CATEGORIES);
  res.json(XN_CATEGORIES);
});

/* ── XN: Posts listing + search + kategori filter + 4 search filter dimensions
   4 filter baru (filterCategory/Time/Views/Duration) mengikuti parameter
   API gateway xchina.tube asli — sebelumnya Vdry hardcode "all" default. ── */
router.get('/api/xn/posts', async (req, res) => {
  ...
  // cat = type=cN format (c0..c21) — kategori per-studio collection
  const cat  = (req.query.cat || '').trim().substring(0, 5);
  // 4 search filter dimensions (hanya berlaku saat q diberikan)
  const filterCategory     = (req.query.fcat    || '').trim().substring(0, 5);   // c0..c21
  const filterTimeInDays   = (req.query.ftime   || '').trim().substring(0, 10);  // 1w/2w/1m/3m/6m/1y
  const filterViews        = (req.query.fviews   || '').trim().substring(0, 10);  // 1k/5k/10k/50k/100k
  const filterDurationInMins = (req.query.fdur   || '').trim().substring(0, 10); // 5/10/30/60

  const cacheKey = `${page}:${q}:${cat}:${filterCategory}:${filterTimeInDays}:${filterViews}:${filterDurationInMins}`;
  ...
  try {
    let raw;
    if (q) {
      // Search dengan keyword user + 4 filter dimensions
      const searchBody = { keywords: q, page };
      if (filterCategory)     searchBody.filterCategory     = filterCategory;
      if (filterTimeInDays)   searchBody.filterTimeInDays   = filterTimeInDays;
      if (filterViews)        searchBody.filterViews        = filterViews;
      if (filterDurationInMins) searchBody.filterDurationInMins = filterDurationInMins;
      raw = await apiPost('/searchSevenVideos', searchBody);
    } else if (cat) {
      // Category filter: type=cN sebagai query param di URL path (BUKAN di body)
      raw = await apiPost(`/sevenVideos?page=${page}&type=${encodeURIComponent(cat)}`);
    } else {
...
router.get('/api/xn/video/:vId', async (req, res) => {
  ...
  try {
    ...
    // Fetch related videos — pakai ?user={creator} (BUKAN ?v=) per API asli
    let related = [];
    const creator = raw.user || raw.creator || raw.upload_user || '';
    if (creator) {
      try {
        const relRaw = await apiPost(`/relatedSevenVideos?user=${encodeURIComponent(creator)}&page=1`);
        related = (Array.isArray(relRaw) ? relRaw : [])
          .map(v => { const n = normVideo(v); return { slug: n.vId, title: n.title, thumb: n.thumb }; })
          .filter(p => p.slug && p.title && p.slug !== vId);
    ...
    const payload = {
      ...
      related,
      creator:     raw.user || raw.creator || raw.upload_user || '',
      vip:         !!raw.vip,
