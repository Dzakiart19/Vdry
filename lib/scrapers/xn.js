...
/* ── Hardcoded category list ──
   xchina.tube tidak punya dedicated categories API — kategori diimplementasikan
   sebagai keyword filter via /searchSevenVideos?keywords=<keyword>.
   Daftar ini diverifikasi: setiap keyword memberikan hasil berbeda dari listing default.
   id = keyword yang dikirim ke /searchSevenVideos; name = label tampilan.
── */
const XN_CATEGORIES = [
  { id: '国产',   slug: 'domestic',    name: 'Chinese (国产)' },
  { id: '日本',   slug: 'japan',       name: 'Japanese (日本)' },
  { id: '欧美',   slug: 'western',     name: 'Western (欧美)' },
  { id: '自拍',   slug: 'selfie',      name: 'Self-filmed (自拍)' },
  { id: '人妻',   slug: 'married',     name: 'Married (人妻)' },
  { id: '学生',   slug: 'student',     name: 'Student (学生)' },
  { id: '制服',   slug: 'uniform',     name: 'Uniform (制服)' },
  { id: '女同',   slug: 'lesbian',     name: 'Lesbian (女同)' },
  { id: '中文字幕', slug: 'subtitled', name: 'Subtitled (字幕)' },
  { id: '无码',   slug: 'uncensored',  name: 'Uncensored (无码)' },
  { id: '有码',   slug: 'censored',    name: 'Censored (有码)' },
  { id: '偷拍',   slug: 'voyeur',      name: 'Voyeur (偷拍)' },
  { id: '按摩',   slug: 'massage',     name: 'Massage (按摩)' },
  { id: '护士',   slug: 'nurse',       name: 'Nurse (护士)' },
  { id: '办公室', slug: 'office',      name: 'Office (办公室)' },
];
...
/* ── XN: Posts listing + search + kategori filter ── */
router.get('/api/xn/posts', async (req, res) => {
  ...
  // cat = Chinese keyword untuk category filter (misal: "国产", "日本")
  const cat  = (req.query.cat || '').trim().substring(0, 30);

  const cacheKey = `${page}:${q}:${cat}`;
  ...
  try {
    let raw;
    if (q) {
      // Search menggunakan keyword user
      raw = await apiPost('/searchSevenVideos', { keywords: q, page });
    } else if (cat) {
      // Category filter: gunakan search endpoint dengan keyword = nama kategori
      raw = await apiPost('/searchSevenVideos', { keywords: cat, page });
    } else {
