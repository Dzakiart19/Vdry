...
function trackRequest(req, _res, next) {
  ...
  else if (p.startsWith('/proxy/rc/stream/')) pushMonitorEvent('rc_video', { id: p.split('/')[4] || '?', ip, ua });
  else if (p.startsWith('/api/rc/posts'))   pushMonitorEvent('rc_posts', { ip, ua });
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .b-rc_video{background:#1a0800;color:#ff6633}
  .b-rc_posts{background:#1a0800;color:#ff6633}
...
function connect() {
  ...
  es.addEventListener('history', e => {
    ...
    // data.events sudah newest-first (di-reverse di server)
    loadHistory(data.events || [], data.totalEvents || 0);
  });

  es.addEventListener('event', e => {
    addLiveEvent(JSON.parse(e.data));
  });
...
function registerMonitorRoutes(app, { getCacheStats }) {
  ...
  app.get('/monitor/log', (req, res) => {
    ...
    // monitorLog: oldest-first; filter yang lebih tua dari before
    const filtered = isFinite(before)
      ? monitorLog.filter(e => e.ts < before)
      : monitorLog.slice();
    const slice   = filtered.slice(-limit);          // ambil N terbaru dari filtered (masih oldest-first)
    res.json({
      events:  slice,
