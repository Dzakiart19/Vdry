/* ══════════════════════════════════════════════════════════════════
   ...
   Tabel dibuat otomatis saat startup via ensureTable() (CREATE TABLE IF NOT
   EXISTS) — tidak perlu migrasi manual, langsung jalan di project fresh.
...
async function ensureTable() {
  try {
    await pool.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_shortlinks_token   ON shortlinks (platform, token);
      CREATE INDEX IF NOT EXISTS idx_shortlinks_expires ON shortlinks (expires_at);
    `);
  } catch (err) {
    console.error('[shortlink] ensureTable error:', err.message);
  }
}
