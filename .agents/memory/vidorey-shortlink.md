...
Route already in `server.js` (not inside any scraper). Validates platform ∈ `{rb,yb,bk,tp,sb}` and
token matches `/^[a-z0-9]{11}$/`, then calls `resolveToken`. Returns `{ slug }` or 404.

**Catatan P5 (tp):** TP terdaftar di platform list tapi tidak aktif memanggil `registerSlug` — TP pakai ID numerik langsung di URL. Route `/api/s/tp/:token` ada di server tapi tidak akan pernah resolve (tidak ada token yang didaftarkan). Ini tidak masalah — route tetap aman karena hanya return 404.
