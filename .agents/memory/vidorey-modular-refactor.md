...
description: How server.js was split into lib/ modules while preserving platform isolation and exact behavior parity. 10 scrapers active (2026-08-02).
---
...
- `lib/scrapers/{p1,rb,yb,bk,tp,sb,vd,xn,zg,er}.js` — one file per platform, each exports `{ router, caches }`; no cross-imports between platform files.
- `server.js` is now a thin composition root (10 scrapers).

**Reality check (2026-08-02):** 10 active scrapers — p1, rb (P2), yb (P3), bk (P4), tp (P5/TikTok), sb (P6), vd (P7), xn (P8), zg (P9), er (P10). "rc" (reddclips) was planned but **never built** — no rc.html or rc.js exists. **UI naming:** Vidorey 1=p1, 2=rb, 3=yb, 4=bk, 5=sb, 6=xn, 7=vd, 8=zg, 9=er; TP is "Vidorey TikTok 1" under "Fitur Lain". UI numbers skip xn/sb internal numbering — just use the ordered list above.
...
**How to apply:** when adding a Platform N, follow the bk.js/er.js pattern — own router + own caches array, no imports from sibling scraper files. `lib/scrapers/*.js` are two directories below project root, so static file paths need `path.join(__dirname, '..', '..', 'public', ...)`.
...
6. **SEMUA 10 HTML files** (index, rb, yb, bk, sb, tp, vd, xn, zg, er + platform baru) — tambah nav drawer item platform baru
